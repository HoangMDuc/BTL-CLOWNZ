...
const App = {
        ...
        fetch("https://62890e4b10e93797c162141e.mockapi.io/clownz" + "/products?sortBy=date&order=desc&p=1&l=4")
            ...
            .then(products => {
                var htmls = products.map(product => {
                    return `
                <div class="col-3">
                    <a href="./san-pham/sp-1.html" class="text-decoration-none text-dark product-card">
    ...
    loadTop() {
        fetch("https://62890e4b10e93797c162141e.mockapi.io/clownz" + "/products?sortBy=date&order=desc&p=1&l=4&catagory_id=3")
            .then(res => res.json())
            .then(products => {
                var htmls = products.map(product => {
                    return `
    ...
    loadBottom() {
        fetch("https://62890e4b10e93797c162141e.mockapi.io/clownz" + "/products?sortBy=date&order=desc&p=1&l=4&catagory_id=4")
            .then(res => res.json())
            .then(products => {
                var htmls = products.map(product => {
                    return `
                <div class="col-3">
                    <a href="./san-pham/sp-1.html" class="text-dark text-decoration-none product-card">
                        <div class="card text-center border-0">
                            <img class="card-img-top"
                                src="./${product.image[0]}"
                                alt="Backbag">
    ...
    loadAccessory() {
        fetch("https://62890e4b10e93797c162141e.mockapi.io/clownz" + "/products?sortBy=date&order=desc&p=1&l=4&catagory_id=5")
            .then(res => res.json())
            .then(products => {
                var htmls = products.map(product => {
...
[
    ...
    },
    {
        ...
        "quantity": 16,
        "image": [
            "/img/accessory/hat/sporty-cap.jpg"
        ],
        "type_id": "9",
        "date": "date 14",
        "title": "CAPS",
